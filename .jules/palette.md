...
## 2026-07-03 - [Semantic Keyboard Shortcuts for Screen Readers]
**Learning:** Visual keyboard shortcut hints (like `<kbd>Cmd + Enter</kbd>`) are often given `aria-hidden="true"` to prevent screen readers from awkwardly spelling them out (e.g., reading "command symbol, return arrow" in the middle of a button label). However, simply hiding the visual hint leaves screen reader users completely unaware that a convenient shortcut exists for the action.
**Action:** When adding keyboard shortcuts to interactive elements (like form submit buttons), always include the `aria-keyshortcuts` attribute (e.g., `aria-keyshortcuts="Meta+Enter Control+Enter"`) on the primary element. This allows screen readers to announce the shortcut semantically in a way that is tailored to the user's assistive technology, while keeping the visual hint cleanly hidden from the screen reader's virtual buffer.

## 2026-07-04 - [Parity for Secondary Action Keyboard Shortcuts]
**Learning:** When a form supports complex, parallel actions (e.g., a primary "Compute V*" action and a secondary "Check DDP" action), providing a semantic keyboard shortcut (`aria-keyshortcuts`) and visual hint (`<kbd>`) only for the primary action leaves screen reader users and power users unaware that secondary actions also have fast paths.
**Action:** Always ensure keyboard shortcut parity for secondary form actions. Explicitly define and listen for alternative shortcuts (like `Shift+Enter`), add the `aria-keyshortcuts="Shift+Enter"` attribute to the secondary button, and provide a visually hidden (but present for sighted users) `<kbd>` hint. This ensures an equitable and robust keyboard navigation experience.
