...
## 2026-05-22 - Fast path for swapped subspace sum
**Learning:** In the `sum_spaces(A, B)` algorithm, orthogonal projection is only used if matrix `A` is strictly orthonormal. If `A` is not orthonormal but `B` is, the algorithm would needlessly fall back to a computationally expensive $O(N^3)$ Rank-Revealing QR factorization (RRQR) on the concatenated matrix `[A, B]`.
**Action:** Since `sum_spaces(A, B)` is mathematically symmetric, add an `elif` condition to check if `B` is strictly orthonormal (`np.linalg.norm(B.T @ B - np.eye(B.shape[1]), ord='fro') < 1e-8`). If it is, swap their roles and compute the projection of `A` onto the orthogonal complement of `B` (`A_perp = A - B @ (B.T @ A)`). This safely prevents the SVD fallback when at least one matrix is an orthonormal basis, yielding significant speedups.

## 2026-05-22 - Numerical drift with pseudo-orthonormal matrices
**Learning:** In iterative algorithms (like computing the $V^*$ subspace), an optimization attempting to avoid the `basis(V @ Y, tol)` operation by directly using `V @ Y` (since the product of two orthonormal matrices is mathematically orthonormal) failed.
**Action:** Do not bypass re-orthogonalization steps in hot loops handling floating point numbers. The theoretical mathematical equivalence does not hold in iterative computations due to accumulated floating-point roundoff errors (numerical drift). Always keep calls to algorithms like `basis()` that act as stabilizers to prevent mathematical regressions.
