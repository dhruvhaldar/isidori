...
## 2024-10-25 - [CRITICAL] Prevent Application-Layer DoS via Nested Function Calls
**Vulnerability:** Attackers could bypass AST complexity checks and cause SymPy to hang (chain-rule explosion DoS) by submitting deeply nested function calls (e.g., `sin(sin(...(x)...))`).
**Learning:** Checking overall polynomial degree and exponent sizes is insufficient for preventing mathematical evaluation DoS. SymPy's differentiation algorithms scale exponentially with function nesting depth. Since CPython allows deep AST nesting (up to ~200 levels), an attacker could easily exploit this without triggering the standard degree limits.
**Prevention:** When parsing user-provided mathematical expressions with `ast.parse` for SymPy evaluation, always traverse the AST recursively to compute the absolute tree depth. Enforce a strict, low depth limit (e.g., `< 50`) on the entire expression tree to explicitly block chain-rule explosion attacks.
