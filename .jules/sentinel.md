...
## 2024-05-24 - [CRITICAL] Prevent Application-Layer DoS via Deeply Nested AST SymPy Evaluation
...
## 2024-05-24 - Rate Limit Bypass via Empty String in X-Forwarded-For
...
## 2024-06-25 - [Application-Layer DoS via Jagged Matrix]
**Vulnerability:** Application-Layer DoS via unhandled jagged matrix validation bypass in Pydantic.
**Learning:** Pydantic models validating nested lists (e.g., `List[List[float]]`) for matrix inputs do not natively enforce that the lists form a rectangular matrix. If jagged matrices (e.g., `[[1, 2], [3]]`) bypass validation and are passed directly to `numpy.array()`, NumPy throws an unhandled `ValueError` which leads to a 400 Bad Request exception loop or even Application-Layer DoS if unhandled at the endpoint level.
**Prevention:** When validating nested list matrices in Pydantic for NumPy consumption, always use a `@model_validator` to explicitly verify that the matrix is rectangular by checking that all rows have the same length as the first row.

## 2024-06-28 - [CRITICAL] Prevent Application-Layer DoS via Wide ASTs
**Vulnerability:** Attackers could bypass AST depth and polynomial degree checks by providing wide, shallow trees (e.g., `max(x, x, x...)` or sequentially chained additions). While these trees did not violate the depth limit (e.g., 50), the sheer number of nodes (e.g., 1,000,000+) caused excessive CPU processing time during `ast.walk` or evaluation phases, leading to an Application-Layer Denial of Service (DoS).
**Learning:** Checking only the depth of an AST is insufficient to prevent CPU exhaustion. Wide and shallow ASTs can still contain an enormous number of nodes that bypass depth-first limits but scale linearly in processing time.
**Prevention:** When validating Python ASTs to prevent SymPy DoS attacks, checking AST depth is insufficient because attackers can bypass it with wide, shallow trees (like chained multiplications). Always use `ast.walk(n)` to count the total number of nodes in the AST subtree and enforce a strict maximum node limit (e.g., 100).
## 2024-06-19 - [CRITICAL] Prevent Application-Layer DoS via Memory Exhaustion (Large Payloads)
**Vulnerability:** Attackers could send massive JSON payloads (e.g., a 1GB string or deeply nested arrays) to endpoints that expect matrices. Because FastAPI and Pydantic attempt to parse the entire request body into memory before validating dimensions, this leads to rapid memory exhaustion and a Denial-of-Service condition for the entire server.
**Learning:** FastAPI does not have a default request size limit. Relying solely on Pydantic's field validation (e.g., checking matrix dimensions after parsing) is insufficient for DoS protection because the memory exhaustion occurs during the parsing phase, before the validation logic runs.
**Prevention:** To prevent Application-Layer DoS via memory exhaustion when FastAPI/Starlette applications parse large JSON payloads, explicitly register a custom request size limit middleware that inspects the `content-length` header and returns a `413 Payload Too Large` for excessively large requests. Register this middleware at the top of the stack (added last in `add_middleware`) so it rejects oversized payloads before they are loaded into memory.

## 2024-06-20 - [CRITICAL] Prevent Application-Layer DoS via Chunked Transfer Encoding
**Vulnerability:** Attackers could bypass the `RequestSizeLimitMiddleware` by omitting the `Content-Length` header and using Chunked Transfer Encoding. The middleware only checked the header, allowing arbitrarily large payloads to be loaded into memory, causing an Application-Layer DoS via memory exhaustion.
**Learning:** Relying solely on the `Content-Length` header for request size limiting is insufficient because HTTP allows streaming payloads (chunked encoding) without specifying the total size upfront.
**Prevention:** Always intercept the raw ASGI request stream (e.g., by wrapping `request.receive`) to keep a running total of bytes received and dynamically abort the request if the cumulative size exceeds the maximum allowed limit.
## 2025-06-21 - [CRITICAL] Prevent Application-Layer DoS via ast.Call Bypass
**Vulnerability:** Attackers could bypass the SymPy Application-Layer DoS constant checks by supplying massive numerical values via function calls (e.g., `float('1e200')` or `complex(1e100, 1e100)`). The AST validation logic only verified that `ast.BinOp`, `ast.UnaryOp`, and `ast.Constant` nodes didn't evaluate to excessively large numbers, missing the outputs generated by `ast.Call` nodes.
**Learning:** Even when restricting AST traversal to a limited set of safe functions, the *output* of those functions must be validated if they can construct massive constants. Functions like `complex` or `float` can generate extremely large numbers that slip through if the `isinstance` type check on AST nodes is too narrow.
**Prevention:** When validating Python ASTs to prevent Application-Layer DoS attacks (e.g., via SymPy's sympify), explicitly validate the outputs of `ast.Call` nodes alongside `ast.BinOp`, `ast.UnaryOp`, and `ast.Constant`. Bypassing this allows attackers to generate massive numbers using built-in functions, causing memory exhaustion or timeouts.
