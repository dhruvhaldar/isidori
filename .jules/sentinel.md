...
## 2024-06-25 - [CRITICAL] Prevent Application-Layer DoS via Unvalidated Matrix Dimensions
**Vulnerability:** Sending jagged matrices (arrays of arrays with inconsistent row lengths) to the API bypassed generic Pydantic size limits and triggered an unhandled `ValueError` when NumPy attempted to construct arrays from them. This allowed remote attackers to easily cause Application-Layer DoS and expose internal stack traces/errors via unhandled exceptions.
**Learning:** Pydantic's `List[List[float]]` type only ensures the input is a list of lists of floats. It does not enforce that the resulting 2D structure is mathematically rectangular. When passing such unvalidated nested lists to `numpy.array()`, NumPy throws a `ValueError` for the inhomogeneous shape, which if uncaught, crashes the request handling layer.
**Prevention:** When accepting nested list matrices (e.g., `List[List[float]]`) via Pydantic for NumPy consumption, always use a `@model_validator` to explicitly verify that the matrix is rectangular (e.g., `any(len(row) != len(mat[0]) for row in mat)`). This ensures jagged arrays are safely rejected with a 422 Unprocessable Entity response before reaching the mathematical engine.
