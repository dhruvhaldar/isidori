...
class RateLimitMiddleware(BaseHTTPMiddleware):
    def __init__(self, app, max_requests: int = 100, window_seconds: int = 60):
        ...
        self.trusted_proxies = set(ip.strip() for ip in os.getenv("TRUSTED_PROXIES", "127.0.0.1").split(",") if ip.strip())
...
class MatrixInput(BaseModel):
    ...
    def check_dimensions(self) -> 'MatrixInput':
        ...
        if len(self.matrix) > 0:
            ...
            if any(len(row) != cols for row in self.matrix):
...
def simulate_system(data: LinearSystemInput):
    ...
    try:
        ...
        # Using np.count_nonzero(F) == 0 instead of not np.any(F) provides ~6x faster zero-matrix checking.
