      ...
      <fieldset disabled={isComputingVStar || isCheckingDDP} aria-invalid={!!vStarError || !!ddpError} aria-describedby={[vStarError ? "vstar-error" : "", ddpError ? "ddp-error" : ""].filter(Boolean).join(" ") || undefined} className="space-y-6 group">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="space-y-2">
                <Label htmlFor="states-n">States (n) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label>
                <Input id="states-n" required type="number" inputMode="numeric" min="1" value={n} onFocus={(e) => e.target.select()} onChange={(e) => setN(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (n === "" || Number(n) < 1) setN(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="inputs-m">Inputs (m) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label>
                <Input id="inputs-m" required type="number" inputMode="numeric" min="1" value={m} onFocus={(e) => e.target.select()} onChange={(e) => setM(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (m === "" || Number(m) < 1) setM(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="outputs-p">Outputs (p) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label>
                <Input id="outputs-p" required type="number" inputMode="numeric" min="1" value={p} onFocus={(e) => e.target.select()} onChange={(e) => setP(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (p === "" || Number(p) < 1) setP(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="disturbances-q">Disturbances (q) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label>
                <Input id="disturbances-q" required type="number" inputMode="numeric" min="1" value={q} onFocus={(e) => e.target.select()} onChange={(e) => setQ(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (q === "" || Number(q) < 1) setQ(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div aria-live="polite" className="space-y-4">
                <Button type="button" onClick={handleCheckDDP} variant="secondary" className="w-full relative" disabled={isCheckingDDP} aria-busy={isCheckingDDP} title="Check DDP Solvability (Cmd/Ctrl + Shift + Enter)" aria-keyshortcuts="Meta+Shift+Enter Control+Shift+Enter">
                  <div className="flex items-center justify-center">
