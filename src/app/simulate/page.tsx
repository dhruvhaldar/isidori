           ...
           <CardContent>
             ...
             >
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="space-y-1"><Label htmlFor="states-n">States (n) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label><Input id="states-n" required type="number" inputMode="numeric" min="1" value={n} onFocus={(e) => e.target.select()} onChange={e => setN(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (n === "" || Number(n) < 1) setN(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} /></div>
                  <div className="space-y-1"><Label htmlFor="inputs-m">Inputs (m) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label><Input id="inputs-m" required type="number" inputMode="numeric" min="1" value={m} onFocus={(e) => e.target.select()} onChange={e => setM(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (m === "" || Number(m) < 1) setM(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} /></div>
                  <div className="space-y-1"><Label htmlFor="outputs-p">Outputs (p) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label><Input id="outputs-p" required type="number" inputMode="numeric" min="1" value={p} onFocus={(e) => e.target.select()} onChange={e => setP(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} onBlur={() => { if (p === "" || Number(p) < 1) setP(1); }} autoComplete="off" autoCorrect="off" spellCheck={false} /></div>
