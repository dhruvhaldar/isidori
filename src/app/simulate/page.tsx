           ...
           <CardContent>
             ...
             >
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-1"><Label htmlFor="outputs-p">Outputs (p) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label><Input id="outputs-p" required type="number" inputMode="numeric" min="1" value={p} onFocus={(e) => e.target.select()} onChange={e => setP(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} autoComplete="off" autoCorrect="off" spellCheck={false} /></div>
                  <div className="space-y-1"><Label htmlFor="disturbances-q">Disturbances (q) <span className="text-red-500" aria-hidden="true">*</span><span className="sr-only">(required)</span></Label><Input id="disturbances-q" required type="number" inputMode="numeric" min="1" value={q} onFocus={(e) => e.target.select()} onChange={e => setQ(e.target.value === "" ? "" : parseInt(e.target.value) || 1)} autoComplete="off" autoCorrect="off" spellCheck={false} /></div>
               </div>
